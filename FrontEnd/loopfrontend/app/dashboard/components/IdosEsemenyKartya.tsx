...
export default function IdosEsemenyKartya({ event, userId }: Props) {
  ...
  const hasAcceptedFoszervezo = allStaff.some(s => s.role === 'főszervező' && s.accepted === true)
  const iAmFoszervezo = allStaff.some(s => s.staff_user_id === userId && s.role === 'főszervező' && s.accepted === true)
  ...
  const renderJelentkezesGomb = () => {
    if (existingApplication === 'loading' || staffLoading) {
      ...
      return (
        <div className="bg-white/5 rounded-xl p-3">
          ...
          <p className="text-white/60 text-sm">
            Szerepkör: <span className="text-white font-semibold">
    ...
    return (
      <button onClick={() => handleApply('szervező')} disabled={applying}
    ...
    <div className="bg-white/10 rounded-xl overflow-hidden">
      <button onClick={() => setOpen(!open)}
        ...
        <div className="flex items-center gap-2 shrink-0">
          {iAmFoszervezo && (
            <span className="bg-yellow-400/20 text-yellow-300 text-xs px-2 py-1 rounded-full font-semibold">👑 Főszervező</span>
          )}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {szorvezokPending.map(s => (
                    <div key={s.id} className="bg-white/10 rounded-xl px-3 py-2">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => handleRejectSzervezo(s.id)}
                        ...
                        <button onClick={() => handleAcceptSzervezo(s.id)}
